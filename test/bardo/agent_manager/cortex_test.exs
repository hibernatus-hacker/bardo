defmodule Bardo.AgentManager.CortexTest do
  ...
  test "cortex sync process works correctly", %{cortex_pid: cortex_pid, a_pids: a_pids} do
    ...
    # Since we're using the same test process as the exoself_pid, we should receive the message
    Process.sleep(100)  # Give some time for the message to be processed
    # Note that the fitness is accumulated to [2.8] = [0.5 + 0.8 + 0.6 + 0.9] and cycles is 2
    assert_receive {:evaluation_complete, ^cortex_pid, [2.8], 2, _time_dif, true}, 100
  end
  
  test "cortex can be reactivated", %{cortex_pid: cortex_pid, exoself_pid: exoself_pid, a_pids: a_pids} do
    ...
    Process.sleep(100)  # Give some time for the message to be processed
    assert_receive {:evaluation_complete, ^cortex_pid, _fitness, _cycles, _time, _goal_reached}, 100
