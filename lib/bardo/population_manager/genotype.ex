defmodule Bardo.PopulationManager.Genotype do
  ...
  require Logger
  alias Bardo.{Models, Functions, Utils, AppConfig, DB}
  ...
  def update_fingerprint(agent_id) do
    ...
    generalized_sensors = Enum.map(s_ids, fn s_id ->
    ...
    generalized_actuators = Enum.map(a_ids, fn a_id ->
    ...
    generalized_pattern = Enum.map(Models.get(a, :pattern), fn {layer_index, ln_ids} ->
      {layer_index, length(ln_ids)}
    end)
    
    generalized_evo_hist = generalize_evo_hist(Models.get(a, :evo_hist))
    ...
    DB.write(Models.set(a, [{:fingerprint, fingerprint}]), :agent)
  end
  ...
  defp get_node_summary([n_id | n_ids], il_acc, ol_acc, ro_acc, function_distribution) do
    ...
    af = Models.get(n, :af)
    il_count = length(Models.get(n, :input_idps))
    ol_count = length(Models.get(n, :output_ids))
    ro_count = length(Models.get(n, :ro_ids))
