defmodule Bardo.PopulationManager.GenomeMutator do
  ...
  alias Bardo.{Models, Utils, AppConfig, DB, LogR}
  alias Bardo.{Plasticity, TuningSelection, Functions}
  alias Bardo.PopulationManager.{Genotype, TotTopologicalMutations}
  ...
  def link_from_element_to_element(generation, from_element_id, to_element_id) do
    case {from_element_id, to_element_id} do
      ...
      {{:neuron, _from_nid}, {:actuator, _to_aid}} ->
        link_from_neuron_to_actuator(generation, from_element_id, to_element_id)
    end
  end
