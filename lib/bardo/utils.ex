defmodule Bardo.Utils do
  ...
  def get_module(module) when is_atom(module) do
    case Bardo.AppConfig.get_env(:build_tool) do
      :erlang ->
        module
      :elixir ->
        module_str = module |> Atom.to_string() |> Macro.camelize() |> to_charlist()
        maybe_add_elixir_prefix(module_str)
    end
  end
  ...
  @spec maybe_add_elixir_prefix(charlist()) :: atom()
  defp maybe_add_elixir_prefix(module) do
    # Using String.starts_with? with charlists doesn't work directly
